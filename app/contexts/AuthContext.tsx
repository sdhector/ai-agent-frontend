/**
 * AuthContext - Migrated for Ignite
 * 
 * Updated imports to use app/services instead of lib
 * Storage functions are now synchronous (MMKV)
 */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAuthStatus = async () => {
    try {
      const token = getToken(); // MMKV is synchronous
      if (!token) {
      ...
      // Try to get cached user data first (MMKV is synchronous)
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.user) {
            ...
            // Save to storage (MMKV is synchronous)
            saveUserId(userData.id);
            saveUserEmail(userData.email);
            saveUserName(userData.name);
            if (userData.picture) saveUserPicture(userData.picture);
    ...
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        if (result.type === 'success' && result.url) {
          ...
          if (token) {
            saveToken(token); // MMKV is synchronous
  ...
  const logout = async () => {
    try {
      ...
      // Clear local data (MMKV is synchronous)
      clearUserData();
      setUser(null);
    }
  };
